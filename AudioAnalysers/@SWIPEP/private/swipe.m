% This is the SWIPE code by Arturo Camacho
...
function [p,t,s] = swipe(x,fs,plim,dt,dlog2p,dERBs,sTHR)
% SWIPE Pitch estimation using SWIPE.
% P = SWIPE(X,Fs,[PMIN PMAX],DT,DLOG2P,DERBS,STHR) estimates the pitch of
...
%
% [P,T,S] = SWIPE(X,Fs,[PMIN PMAX],DT,DLOG2P,DERBS,S/thr) returns the times
% T at which the pitch was estimated and their corresponding pitch strength.
%
% P = SWIPE(X,Fs) estimates the pitch using the default settings PMIN =
% 30 Hz, PMAX = 5000 Hz, DT = 0.01 s, DLOG2P = 1/96 (96 steps per octave),
% DERBS = 0.1 ERBs, and STHR = -Inf.
%
% P = SWIPE(X,Fs,...[],...) uses the default setting for the parameter
...
% xlabel('Time (ms)')
% ylabel('Pitch (Hz)')
if ~ exist( 'plim' ) | isempty(plim), plim = [30 5000]; end
if ~ exist( 'dt' ) | isempty(dt), dt = 0.01; end
if ~ exist( 'dlog2p' ) | isempty(dlog2p), dlog2p = 1/96; end
